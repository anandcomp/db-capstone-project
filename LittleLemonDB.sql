...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`developer`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `ordersview2` AS select `customer`.`CustomerID` AS `CustomerID`,`customer`.`FullName` AS `FullName`,`orders`.`OrderID` AS `OrderID`,`orders`.`TotalCost` AS `TotalCost` from (`customer` join `orders`) where (`orders`.`TotalCost` > 150) order by `orders`.`TotalCost` desc */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2024-12-18 11:54:19
